...
def print_analyse_VCSEL_lifetime_tuning(VCSEL_cavity_tuning_properties):
    ...
    print(
        f"Mirror loss tuning range by AR coating: {VCSEL_cavity_tuning_properties.alpha_m_coating_range*1e-2:.4f} cm^-1"
    )
    print(
        f"Mirror loss tuning range by etching: {VCSEL_cavity_tuning_properties.alpha_m_etch_range*1e-2:.4f} cm^-1"
    )
    print(
...
def plot_analyse_VCSEL(analyse_VCSEL_results, Save_to="pdf"):
    ...
    ax5.grid(which="major", linestyle="-", linewidth=0.4)
